...
const {nasa} = require('./services/nasaApiServices')
...
client.on('ready',async () => {
    ...
    await client.sendMessage(element,schemas.botInit.text)
})
...
client.on('message_create',async message => {
    ...
    function sendReply(Msg,title = ''){
		message.reply(Msg,message.from,{caption:title,extra:{
            hd:true
        }});
    }
	if (msg === '!bot on') {
        ...
        client.sendMessage(msg.from, button);
         ...
         const timer = (number) => new Promise((res) => setTimeout(res,number))
         const sentMsg = await client.sendMessage(message.from,'Procesando...');
         await timer(1000);
       
         for (let i = 1; i <= 10; i++) {
           const progress = i * 10;
           const filledBars = '▓'.repeat(i);
           const emptyBars = '▒'.repeat(10 - i);
           await sentMsg.edit(`${filledBars}${emptyBars} ${progress}%`);
           await timer(500);
         }
           ...
           // await this.sendLoadingBar(chat);
            ...
            const response = message.hasQuotedMsg
            if(response){
                let chat = message.getChat()
                let h = await message.getQuotedMessage()
                ...
                if(h.id.fromMe){
                    ...
                    if(text.includes('class-nvs')){
                        ...
                        const id = searchID[0].replace('[','').replace(']','').trim()
                        console.log(id)

                        const data = await axios.get(`http://localhost:1024/services/nvi?q=${id}`)
...
|-------------------->
            ...
            ${chapterList}
                        ...
                        sendReply('descaragando pdf de '+id+' 📄 Antes de enviar el PDF, queremos informarte de que el proceso puede tardar entre 1 y 2 minutos. Por favor, ten paciencia durante este tiempo. Si después de esperar este período no recibes nada, por favor inténtalo de nuevo. ¡Gracias por tu comprensión! 🕒')

                        const media = await MessageMedia.fromUrl(`http://localhost:1024/services/pdf/${id}`,{unsafeMime:true,sendAsDocument:true})
                        ...
                        const id=select[0].replace('{','').replace('}','').trim()
                        sendReply(`🤖Porfavor espere un momento mientras s  e descarga su audio...`);
                        const Audio = await MessageMedia.fromUrl('http://localhost:1024/services/ytmp3?ytdLink='+id,{unsafeMime:true});
                        sendReply(Audio)
...
client.on('group_leave', async (notification) => {
    ...
    console.log('--------------------')
    const profile =await client.getProfilePicUrl(notification.id.id+'@g.us')
    console.log(profile)
    console.log(c)
});
...
client.on('message_revoke_everyone', async (after, before) => {
    ...
    if (before.type === 'image') {
            console.log(before)
            const media = await new MessageMedia(before._data.mimetype,before._data.body)
