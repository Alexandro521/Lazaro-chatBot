...
import{ Client,MessageMedia,GroupNotificationTypes, LocalAuth} from 'whatsapp-web.js';
...
import {clientFunc} from './functions/clientFunctions.js'
...
client.on('message_create',async message => {
    try {
        ...
        if (msg === '!bot on') {
            ...
            if(!message.hasMedia){
                message.reply('🤖 Debes enviar una imagen para que funcione el comando')
            }
            ...
            const IsResponse = message.hasQuotedMsg
                ...
                if (responseMsg.id.fromMe) {
                    ...
                    if (searchText && searchText.length > 1) {
                        ...
                        if (text.includes('class-nvs')) {
                            ...
                            let headerText = schemas.mangaHeaderInfo({mangaData});
                            let chapterList = ''
                            element.chaptersList.forEach((chapterData,index) => {
                                chapterList += schemas.mangaChapterSchemaInfo({chapterData,index})
                            })
