...
export class Commands {
  ...
  static async StickerCreate({ Message }: { Message: Message }) {
    if (!Message.hasMedia) {
      await Message.reply(
        "🤖 Debes enviar una imagen para que funcione el comando"
      );
      return;
    }
  ...
  static async _testName({ Message }: { Message: Message }) {
    ...
    if (text.includes("class-nvs")) {
      ...
      await Message.reply(media, await Message.from, {
        caption: `
        ${headerText}
        ${chapterList}
                    `,
      ...
      await Message.reply(
        "Archivo descargado con !exito, Enviando...",
        Message.from
      );
  ...
  static async main({ Message }: { Message: Message }) {
    try {
      await Message.react("📜");
  ...
  static async Everyone({ Message }: { Message: Message }) {
    try {
      await Message.react("🗣");
      const chat = <GroupChat>await Message.getChat();
      if (!chat.isGroup) {
        Message.reply(
          "Este comando solo puede ser usado enviado en grupos",
          Message.from
        );
      }
      let text = "";
  ...
  static async test_gay({ message }: { message: Message }) {
    try {
      message.react("🏳️‍🌈");
