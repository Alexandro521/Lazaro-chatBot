...
import { main,menu1,menu2,menu3,menu5,menu6 } from "../schemas/textSchemas";
export async function onMessageCreate(message: Message) {
  try {
    const chat = await message.getChat();
    messageInfo(message, chat);
    if (message.body === "!main") {
      const media = await MessageMedia.fromFilePath(process.cwd() + "/static/img/bot.webp")
    ...
    else if (message.body.startsWith("!")) await commandExec(message.body, message);
